 ...
 */
public class Problem_02_BiggestBSTTopologyInTree {
    ...
    /*
        暴力解法
        首先明确对任何一个拓扑结构来说，都必须包含一个头节点，那么就遍历二叉树上的每个节点，依次让每个
        节点做拓扑结构的头节点，得到每个节点下的情况，记录下最大节点数。
        如何得到每个节点下的拓扑情况？
        首先判断某个节点对于头节点来说，能不能加入到头节点的拓扑结构中。判断过程：拿这个节点的值
        从头节点的位置进入，按照搜索二叉树的规则往下找，看看能不能回到这个节点原来的位置，如果能就
        说明这个节点可以加入到头节点的拓扑结构中；如果不能回到，就不能加入。
    */
    public static int bstTopoSize1(Node head) {
        if (head == null) {
            return 0;
        }
        int max = maxTopo(head, head);                      //先序遍历二叉树
